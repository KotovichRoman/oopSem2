...
namespace lab4
{
    public partial class Flats : Form
    {
        ...
        private void ProxyGetInfoButton_Click(object sender, EventArgs e)
        {
            Flat flat = new Flat(addres);
            getObj(flat);

            ISubject proxy = new Proxy.Proxy(flat);
            proxy.GetInfo();
        }
        ...
        private void SaveButton_Click(object sender, EventArgs e)
        {
            Flat flat = new Flat(addres);
            getObj(flat);
            if (!String.IsNullOrEmpty(flat.Addres.Country))
            {
                history.history.Push(flat.SaveState());
            }
            else
            {
        ...
        private void HistoryButton_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(flat.Addres.Country))
            {
                flat.RestoreState(history.history.Pop());
            }
            else
            {
                MessageBox.Show("Выберите квартиру!");
